...
export const CreateAnimal = () => {
  ...
  const handleSubmit = async () => {
    if (animal === null) {
      if (name && breed && hoursTrained && month && day && year && notes) {
        let res = await createAnimal(name, breed, hoursTrained, month, day, year, notes);
        if (res.status === 200) {
          setName("");
          setBreed("");
          setHoursTrained("");
  ...
  const createAnimal = async (name, breed, hoursTrained, month, day, year, notes) => {
        ...
        <div className="flex w-full justify-center">
          <div className="items-start w-1/2">
            ...
            <div className="flex mt-4">
              ...
              <button className="bg-red-500 text-white font-medium px-4 py-2 rounded-md" onClick={handleSubmit}>
                Save
              </button>
            </div>
          </div>
